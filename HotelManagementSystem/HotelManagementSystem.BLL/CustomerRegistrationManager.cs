    ...
    public class CustomerRegistrationManager
    {
        ...
        public string SaveCustomerInRegisteredRoom(bool isNewCustomer, Customer oCustomer, RoomServiceToCustomer oRoomServiceToCustomer)
        {
            ...
            try
            {
                    ...
                    if (status)
                    {
                        status = RoomService.UpdateRoomStatus(oRoomServiceToCustomer.RoomCode);
                        return "Customer with customer ID " + oCustomer.CustomerCode + ", is registered at room number " + oRoomServiceToCustomer.RoomCode.ToString();
                    }
                    return "null";
